...
@main_router.get("/user_credits/{user_id}")
async def user_credits(user_id: int, token: Annotated[str, Depends(settings.auth2_scheme)], db: AsyncSession = Depends(get_session)) -> list[ClosedUserCreditResponse | OpenedUserCreditResponse]:
...
@main_router.get("/plans_performance")
async def plans_performance(date: datetime.date, token: Annotated[str, Depends(settings.auth2_scheme)], db: AsyncSession = Depends(get_session)) -> list[PlanPerformanceResponse]:
...
@main_router.get("/year_performance")
async def year_performance(year: int, token: Annotated[str, Depends(settings.auth2_scheme)], db: AsyncSession = Depends(get_session)) -> list[MonthPerformanceResponse]:
    plan_service = PlanService(db)
    credit_service = CreditService(db)
    payments_service = PaymentService(db)
...
@main_router.post("/plans_insert")
async def plans_insert(file: Annotated[bytes, File()], token: Annotated[str, Depends(settings.auth2_scheme)], db: AsyncSession = Depends(get_session)):
